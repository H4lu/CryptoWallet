...
export function sendTransaction(currency: string, paymentAddress: string, amount: number, fee: number, redirect: any, course: number, balance: number) {
  ...
  case 'bitcoin':
    ...
    handle(paymentAddress, amount, fee, redirect, course, balance)
    break
  case 'litecoin':
    console.log('AMOUNT IN SEND LTC', amount)
    handleLitecoin(paymentAddress, amount, fee, redirect, course, balance)
    break
  case 'ethereum':
    console.log('AMOUNT IN SEND ETH', amount)
    handleEthereum(paymentAddress, amount, fee, 21000, redirect, course, balance)
    break
  }
}
